<script lang="ts">
  ...
  async function checkTrigger() {
    ...
    if (carXPercent <= TRIGGER_THRESHOLD) {
      ...
      // 🚀 The library's built-in modal handles everything
      const result = await BlsquiSDK.requestTransaction({
        useDefaultModal: true,
        isTestnet: true,
        flixId: 'your-custom-flix-id',
        args: {
      ...
      if (['SEALED', 'EXECUTED', 'FINALIZED', 'SUCCESS'].includes(result.status)) {
        ...
        autoDriveExit = true; // Drive off into arena
      } else if (result.status === 'CANCELED') {
        statusMessage = 'Tournament entry canceled.';
        // Bounce car 30px / ~4.5% to the right
        carXPercent = Math.min(90, carXPercent + 4.5);
      } else {
        statusMessage = `Transaction ${result.status.toLowerCase()}: ${result.error || ''}`;
        carXPercent = Math.min(90, carXPercent + 4.5);
      }
    }
  }
...
<style>
  ...
  .cyber-car {
    ...
    object-fit: contain;
    filter: drop-shadow(0 15px 30px rgba(0, 0, 0, 0.8)) drop-shadow(0 0 20px rgba(6, 182, 212, 0.3));
    animation: idle-hover 3s ease-in-out infinite;
  }

  .driving-fast {
    animation: none;
    filter: drop-shadow(-20px 0 25px rgba(6, 182, 212, 0.6));
  }

  @keyframes idle-hover {
    0%, 100% { transform: translateY(25%); }
    50% { transform: translateY(calc(25% - 8px)); }
  }
